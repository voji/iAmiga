...
{
    float marginx = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? 95 : 66);
    float marginy_bot = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ?10:7);
...
- (void)setAutoHide:(BOOL)b
{
    ...
    if (b)
    {
        [btnAutoHide setImage:[UIImage imageNamed:UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? @"unsticky~ipad.png":@"unsticky.png"]
                     forState:UIControlStateNormal];
        [self performSelector:@selector(hideContent) withObject:nil afterDelay:autoHideInterval];
    }
    else
    {
        [btnAutoHide setImage:[UIImage imageNamed:UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad ? @"sticky~ipad.png":@"sticky.png"]
                     forState:UIControlStateNormal]; 
        [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(hideContent) object:nil];
    }
}
